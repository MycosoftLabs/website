 ...
 * redirects to /natureos/ancestry/species/{id} for the full detail view.
 */
    ...
    fetch(
      ...
      .then((d) => {
        ...
        if (taxon?.id) {
          ...
          router.replace(`/natureos/ancestry/species/${taxon.id}`)
        } else {
          // Nothing found — show explorer search as fallback
          router.replace(`/natureos/ancestry/explorer?search=${encodeURIComponent(name)}`)
        }
      })
      .catch(() => {
        router.replace(`/natureos/ancestry/explorer?search=${encodeURIComponent(name)}`)
