...
/**
 * Fusarium-local copy of the NatureOS Fungi Compute desktop gate.
 *
 * The application composition remains unchanged. The only intentional route
 * difference is that the small-screen back action stays inside Fusarium.
 */
export default function FusariumFungiComputeLayout({ children }: { children: React.ReactNode }) {
  useEffect(() => {
    ...
    <FusariumTwinSurface>
      <div className="lg:hidden flex min-h-dvh flex-col items-center justify-center bg-black text-white px-6 py-16 text-center">
        <div className="max-w-sm mx-auto space-y-6">
          <svg
            viewBox="0 0 80 40"
            className="h-16 w-32 mx-auto text-green-400"
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            strokeLinecap="round"
          ...
          <div className="flex flex-col gap-3">
            <Link
              href="/fusarium"
              className="flex items-center justify-center h-11 px-6 rounded-lg bg-green-600 text-white font-medium text-sm"
            >
              ← Back to Fusarium
            </Link>
            <button
      ...
      <div className="hidden lg:block fixed inset-0 bg-black overflow-hidden z-50">{children}</div>
    </FusariumTwinSurface>
  )
}
