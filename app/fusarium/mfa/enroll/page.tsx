...
import { FusariumMfaEnrollForm } from "../FusariumMfaEnrollForm"

export const metadata: Metadata = {
  title: "Fusarium Owner 2FA Enrollment | Mycosoft",
  robots: { index: false, follow: false },
}

export const dynamic = "force-dynamic"

export default async function FusariumMfaEnrollPage({
  searchParams,
}: {
  ...
  if (auth.mfaRequired) {
    redirect(`${FUSARIUM_MFA_CHALLENGE_PATH}?redirectTo=${encodeURIComponent(safeNext)}`)
  }
  ...
  return (
    <main className="flex min-h-dvh items-center justify-center bg-black px-4 py-10">
      <FusariumMfaEnrollForm redirectTo={safeNext} />
    </main>
  )
}
