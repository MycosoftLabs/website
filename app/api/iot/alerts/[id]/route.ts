...
async function proxyToMas(request: NextRequest, path: string, method: string) {
  ...
  const body =
    method === "PATCH" ? await request.json().catch(() => ({})) : undefined
...
export async function GET(
  ...
  return proxyToMas(request, `/api/iot/alerts/${alertId}`, "GET")
}
...
export async function PATCH(
  ...
  const alertId = context.params.id
  return proxyToMas(request, `/api/iot/alerts/${alertId}`, "PATCH")
}
...
export async function DELETE(
  ...
  const alertId = context.params.id
  return proxyToMas(request, `/api/iot/alerts/${alertId}`, "DELETE")
}
