...
async function proxyToMas(request: NextRequest, path: string, method: string) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      alerts: [],
      rules: [],
      error: error instanceof Error ? error.message : "MAS IoT alerts are unreachable",
    })
  }
}

export async function GET(request: NextRequest) {
  return proxyToMas(request, "/api/iot/alerts", "GET")
}

export async function POST(request: NextRequest) {
  return proxyToMas(request, "/api/iot/alerts", "POST")
}
