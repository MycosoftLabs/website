...
function mergeMasDevice(
  ...
  const telemetry = extra.latest_telemetry ?? existing?.telemetry ?? null;
  const telemetryUseful = hasUsefulTelemetry(telemetry);
  const expectedFieldStatus =
    existing?.status === "online" || existing?.status === "connected"
      ? existing.status
      : "connected";
  byId.set(field?.catalog_id || id, {
    ...
    status: field && !telemetryUseful ? expectedFieldStatus : toEarthSimConnectionStatus(String(d.status ?? "")),
    location: loc,
    location_label:
    ...
    telemetry,
    source: field && !telemetryUseful ? existing?.source ?? "field" : "mas",
    agent_url: agentUrl,
    host,
  });
}
...
async function buildDevicesPayload(): Promise<EarthSimDevicesPayload> {
    ...
    for (const probe of operatorProbes) {
      ...
      byId.set(field.catalog_id, {
        ...
        for (const device of localMycoBrainDevices) {
          ...
          const existing = byId.get(rowId);
          const telemetry = d.telemetry ?? existing?.telemetry ?? null;
          const nextStatus = toEarthSimConnectionStatus(String(d.status ?? ""));
          const expectedFieldStatus =
            existing?.status === "online" || existing?.status === "connected"
              ? existing.status
              : "connected";
          const status =
            field && !hasUsefulTelemetry(telemetry) && (nextStatus === "offline" || nextStatus === "stale")
              ? expectedFieldStatus
              : nextStatus;
          byId.set(rowId, {
            ...(existing || {}),
    ...
    await Promise.all(
      FIELD_MYCOBRAIN_DEPLOYMENTS.map(async (field) => {
        const current = byId.get(field.catalog_id)
        if (!current || hasUsefulTelemetry(current.telemetry)) return

        let telemetry = await fetchNetworkTelemetrySnapshot(field.registry_id)
        if (!hasUsefulTelemetry(telemetry) && field.mdp_device_id !== field.registry_id) {
          telemetry = await fetchNetworkTelemetrySnapshot(field.mdp_device_id)
        }
        if (!telemetry || !hasUsefulTelemetry(telemetry)) return

        byId.set(field.catalog_id, {
          ...current,
          telemetry,
          status: "connected",
          source: current.source === "field" ? "mas" : current.source,
          lastSeen: String(
            telemetry.captured_at || telemetry.timestamp || telemetry.last_seen || current.lastSeen || new Date().toISOString()
          ),
        })
      })
    )
