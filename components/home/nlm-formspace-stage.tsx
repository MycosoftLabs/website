...
const CARDS = [
  {
    ...
    image: "/assets/fusarium/nlm-intelligence-core.jpg",
    imageAlt: "Nature Learning Model",
  ...
  },
  {
    ...
    image: "/assets/fusarium/sensing-band-dark.jpg",
    imageAlt: "FormSpace",
      ...
      {CARDS.map((card) => (
        ...
        >
          ...
          <Link href={card.href} className="relative flex flex-1 flex-col text-center">
            <div className="relative h-28 w-full overflow-hidden rounded-t-2xl sm:h-32">
              <Image
                src={card.image}
                alt={card.imageAlt}
                fill
            ...
            <div className="flex flex-1 flex-col p-4 text-center sm:p-5">
              <p className="text-sm font-semibold uppercase tracking-[0.16em] text-slate-500 dark:text-white/70">{card.index}</p>
              <h2 className="mt-1 text-xl font-semibold text-slate-950 dark:text-white">{card.title}</h2>
              <p className="mt-1 text-sm text-slate-600 dark:text-white/70">{card.subtitle}</p>
              <p className="mt-2 text-xs leading-4 text-slate-800 dark:text-white/90">{card.body}</p>
            </div>
          </Link>
