...
import { RING_FACES, type BuoyTelemetry, type CameraFeed } from "@/lib/psathyrella/contract";
...
import { useContactIntel } from "@/lib/psathyrella/useContactIntel";
import { useMotionDetect } from "@/lib/psathyrella/useMotionDetect";
import { useModelComparison } from "@/lib/psathyrella/useModelComparison";
import { useMagnetometer } from "@/lib/psathyrella/useMagnetometer";
import { fuseModels } from "@/lib/psathyrella/modelFusion";
import ModelComparisonPanel from "@/components/psathyrella/camera/ModelComparisonPanel";
...
function useHudSensors(active: boolean) {
  ...
  const { data: tof } = useSWR(active ? "/api/psathyrella/fusion-sensors/points" : null, probe, opts);
  const { data: mmw } = useSWR(active ? "/api/psathyrella/fusion-sensors/mmwave" : null, probe, opts);
...
export default function Quad360View({
  ...
  // ── AI detections (dark unless NEXT_PUBLIC_PSATHYRELLA_CAM_AI=1). Boxes arrive in PIXELS on the
