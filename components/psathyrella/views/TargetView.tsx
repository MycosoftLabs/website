...
import type { BuoyCommand, BuoyTelemetry, CameraFeed, IrCutMode } from "@/lib/psathyrella/contract";
import { cssFilter, DEFAULT_FILTER_STATE, isFilterActive, filterSummary, type VideoFilterState } from "@/lib/psathyrella/videoFilters";
import FilterControls from "@/components/psathyrella/camera/FilterControls";
import DetectionOverlay from "@/components/psathyrella/camera/DetectionOverlay";
import { useCameraDetections } from "@/lib/psathyrella/useCameraDetections";
import { detectionBearing, planFollow, reacquire, type LockedTrack, type FollowMode } from "@/lib/psathyrella/targetTracking";
import { classifyDetection, shouldSurface, groupColor } from "@/lib/psathyrella/maritimeOntology";
import { useStableDetections } from "@/lib/psathyrella/useStableDetections";
import { pointFromEvent, designateAt } from "@/lib/psathyrella/designate";
import SensorStatsPanel, { type SensorStats } from "@/components/psathyrella/camera/SensorStatsPanel";
import MaritimeHud, { type HudContact } from "@/components/psathyrella/camera/MaritimeHud";
import VisorHud, { type VisorContact } from "@/components/psathyrella/camera/VisorHud";
import RelabelPopover from "@/components/psathyrella/camera/RelabelPopover";
...
export default function TargetView({
  ...
  /*
   * IR-cut day/night — posts to the owner-gated proxy at /api/psathyrella/camera/ircut.
   *
  ...
  const setIrCut = async (mode: IrCutMode) => {
    ...
    setIrCutMode(mode);
    try {
      const res = await fetch("/api/psathyrella/camera/ircut", {
        method: "POST",
        ...
        <div className="relative aspect-video max-h-full w-full min-w-0 overflow-hidden bg-black"
          ...
          {src && (
            <div className="absolute inset-0" style={{ transform: `scale(${zoom}) rotate(${rotateDeg}deg)`, transformOrigin: `${50 + pan.x * 45}% ${50 + pan.y * 45}%`, transition: "transform 0.18s ease-out" }}>
              ...
              <img ref={frameRef} key={src} src={src} alt="Psathyrella target camera (IMX477)" className="h-full w-full object-contain"
