...
export function TopNav() {
  ...
  return (
    <>
      <header className="natureos-top-nav h-12 md:h-14 border-b border-black/10 bg-white/85 text-black shadow-sm shadow-black/5 backdrop-blur-xl dark:border-white/12 dark:bg-black/55 dark:text-white sticky top-0 z-40">
        <div className="flex h-full items-center px-3 md:px-4 gap-x-1 md:gap-x-4 container mx-auto">
          ...
          <Button
            ...
            size="icon"
            onClick={() => router.back()}
            className="text-black/70 hover:text-black dark:text-gray-400 dark:hover:text-white"
          >
            <ArrowLeft className="h-5 w-5" />
          </Button>

          {/* Logo/Home */}
          <Link href="/natureos/dashboard" className="flex items-center gap-1 md:gap-2 font-semibold text-black hover:text-blue-700 transition-colors dark:text-white dark:hover:text-blue-400">
          ...
          <form onSubmit={handleSearch} className="flex-1 max-w-xl ml-2 md:ml-8 hidden sm:flex items-center gap-2">
            <div className="relative flex-1">
              <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-black/50 dark:text-gray-400" />
              <Input
                ref={searchInputRef}
                ...
                className="pl-10 pr-20 border-black/15 bg-white/80 text-black placeholder:text-black/50 focus-visible:ring-blue-500 dark:bg-gray-900 dark:border-gray-800 dark:text-white dark:placeholder:text-gray-500"
              />
              {matchingSearchTargets.length > 0 && (
                <div className="absolute left-0 right-0 top-[calc(100%+0.35rem)] z-50 rounded-md border border-black/10 bg-white/95 p-1 shadow-xl backdrop-blur dark:border-gray-800 dark:bg-gray-950/95">
                  {matchingSearchTargets.map((target) => (
                    <button
                      ...
                      className="flex w-full items-center justify-between rounded px-3 py-2 text-left text-sm text-black/80 hover:bg-blue-500/15 hover:text-black dark:text-gray-200 dark:hover:text-white"
                    >
                      <span>{target.label}</span>
                      <span className="text-xs text-black/50 dark:text-gray-500">{target.href.replace("/natureos/", "")}</span>
                    </button>
                  ))}
                </div>
              )}
              <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                <kbd className="hidden md:inline-flex items-center px-1.5 py-0.5 text-[10px] text-black/50 bg-white/80 border border-black/10 rounded dark:text-gray-500 dark:bg-gray-800 dark:border-gray-700">
                  Ctrl+K
                </kbd>
                <Button type="submit" variant="ghost" size="icon" className="h-6 w-6 text-black/60 hover:text-blue-700 dark:text-gray-400 dark:hover:text-blue-400">
          ...
          <div className="flex items-center gap-1 md:gap-2 ml-auto">
            ...
            <Button
              variant="ghost"
              size="icon"
              className="sm:hidden text-black/70 hover:text-black dark:text-gray-400 dark:hover:text-white"
                ...
                <div className="space-y-3 mt-4">
                  ...
                  <div className="p-3 border rounded-lg bg-blue-500/5 border-blue-500/20">
                    ...
                    <ul className="mt-2 space-y-1 text-sm text-muted-foreground">
                      ...
                      <li>2. Navigate using the sidebar menu to access Science, SI, Tools, and more</li>
