...
export function SidebarGroupLabel({ className, children, ...props }: SidebarGroupLabelProps) {
  ...
  return (
    <div className={cn("px-2 py-1 text-xs font-medium text-gray-400 uppercase tracking-wider", className)} {...props}>
      {children}
    </div>
  )
}
      ...
      {asChild ? (
        ...
        >
          <div className="flex items-center">
            {React.Children.map(children, (child) => {
              if (React.isValidElement(child)) {
                return React.cloneElement(child, { className: cn(child.props.className, "mr-2") })
              }
              return null
            })}
            <span>{React.Children.toArray(children)[1]}</span>
          </div>
        </Comp>
      )}
      {!isOpen && tooltip && (
        <div className="absolute left-full top-1/2 -translate-y-1/2 ml-2 px-2 py-1 rounded bg-gray-800 text-xs text-white opacity-0 group-hover:opacity-100 transition-opacity z-50 whitespace-nowrap">
          {tooltip}
        </div>
      )}
    </div>
