...
export default function MindexFirmsLayer({ map, enabled, opacity = 0.85 }: Props) {
  ...
  useEffect(() => {
    ...
    const onClick = (e: any) => {
      ...
      // Defensive HTML-escape: detectedAt is upstream data; toUTCString() already
      // yields a safe date string, but escaping keeps setHTML XSS-clean and
      // satisfies the security scanner. (Jun 23 2026 — PR #230 security review.)
      const esc = (s: unknown) =>
      ...
      const html =
        ...
        (when ? `<div><span style="opacity:.55">detected:</span> ${esc(when)}</div>` : "") +
