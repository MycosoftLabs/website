...
export default function ITDXReplayDock(){
 ...
 useEffect(()=>{if(!state.playing)return;const timer=setInterval(replay.tick,500);return()=>clearInterval(timer)},[state.playing])
 useEffect(()=>()=>replay.pause(),[])
 const frame=snapshot(state.index),asset=frame.assets.find(a=>a.id===state.selected)!
 ...
 return <aside className={styles.dock} aria-label="ITDX fictional replay controls">
   ...
   {open&&<>
    ...
    <p className={styles.muted}>Four invented markers and preset paths near 0°N, 0°E.</p>
