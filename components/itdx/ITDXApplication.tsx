...
export default function ITDXApplication(){
 ...
 return <section className={`${styles.workspace} ${styles.application}`}>
  ...
  <p>v1.4 cite path is MAS <code>http://192.168.0.188:8001</code>, MINDEX <code>http://192.168.0.189:8000</code>, and Google Maps traffic/pathways. Optional 8765/8766 lab HTML is extra chrome, not a gate. Earth Sim overlay lives on /fusarium/earth-simulator.</p>
  <div className={styles.row}><strong>ITDX backend: {backend.status==='NOT_SUPPLIED'?'v1.4 SUPPLIED · optional lab NOT_SUPPLIED':backend.status}</strong><span>{backend.version?'v'+backend.version:''} {backend.documents!==undefined?backend.documents+' documents':''}</span><span>Fusarium runtime: {runtime}</span><button onClick={()=>setCounter(c=>c+1)}>Refresh connections</button><Link href="/fusarium/earth-simulator">Earth Simulator</Link></div>
  <p className={styles.muted}>Selected run: {context.runId||'none'} · dataset: {context.datasetId||'none'} · origin: {context.dataOrigin}. Selection references are available to registered Fusarium consumers; they do not grant access or run another app. Google Maps Directions/Matrix/Geocode remain SUPPLIED on the website BFF.</p>
  ...
  {selected&&backend.status!=='CONNECTED'&&<article className={styles.card} data-testid="itdx-lab-status"><h2>{backend.status==='CHECKING'?'ITDX lab is still connecting':backend.status==='NOT_SUPPLIED'?'Optional algorithm lab NOT_SUPPLIED':'ITDX lab '+backend.status}</h2><p>{backend.message||'Checking the optional 8765/8766 lab…'}</p><p>ITDX v1.4 tabs stay visible. MAS and MINDEX remain the live cite path. Optional algorithm lab HTML is not framed when unbound so a JSON error is not printed as “not configured properly.”</p><p className={styles.muted}>No synthetic success, no invented p, no live COP. Weka / situation / Task 8 below stay honest.</p></article>}
