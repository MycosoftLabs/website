...
import {
  ...
  type BuoyTelemetry,
  type RadioKind,
} from "@/lib/fusarium/gcs/contract";
import { Panel, SectionLabel, StatLED, Readout, Bar, TacButton } from "@/components/fusarium/gcs/ui";
import { useEdgeHealth } from "@/lib/fusarium/gcs/useEdgeHealth";
...
export function CommsPanel({
  ...
  const runBurst = async () => {
    setBursting(true);
    try { setBurst(await fetch("/api/fusarium/gcs/link-status?burst=5", { cache: "no-store" }).then((r) => r.json())); }
    catch { setBurst(null); }
  ...
  const uwPost = async (action: string, params: Record<string, unknown> = {}) => {
    setUwBusy(true);
    try {
      await fetch("/api/fusarium/gcs/acoustic", {
        method: "POST",
