...
import "@/lib/fusarium/gcs/domGuard"; // null-safe removeChild — last-line defense vs the MapLibre↔React commit race
...
import { ArrowLeft, Camera, Layers, Map as MapIcon, FlaskConical, Navigation, Radio, Waves, Route, Moon, Sun, Monitor, Hand, Bot } from "lucide-react";
import { VIEW_MODES, type ViewMode, type Waypoint, type SelectedDevice, type MissionPlan, primaryBuoySelection, PSATHYRELLA_DEVICE_ID } from "@/lib/fusarium/gcs/contract";
import { useBuoyTelemetry } from "@/lib/fusarium/gcs/useBuoyTelemetry";
import { useSessionRecorder } from "@/lib/fusarium/gcs/useSessionRecorder";
import { useControlSession } from "@/lib/fusarium/gcs/useControlSession";
...
import { GLOBAL_CONTROL_DEVICE_PROFILES } from "@/lib/fusarium/gcs/device-profiles";
...
export function FusariumPsathyrellaConsole({
  profileId,
  onProfileChange,
}: {
  profileId: "psathyrella";
  ...
  const [simulated, setSimulated] = useState(() => process.env.NEXT_PUBLIC_FUSARIUM_GCS_PSATHYRELLA_SIM === "1");
    ...
    <div className={`psa-console dark [color-scheme:dark] fixed inset-0 z-[60] flex flex-col overflow-hidden bg-[#04070e] font-sans text-slate-200 ${display.rootClass}`}>
      ...
      <header className="psa-glass-strong z-[70] flex shrink-0 items-center justify-between gap-2 border-b border-white/10 px-4 py-2">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <label className="hidden items-center gap-1 text-[9px] uppercase tracking-wider text-slate-500 md:flex">
            Profile
            <select
              value={profileId}
              onChange={(event) => onProfileChange(event.target.value as "psathyrella" | "agaric" | "mushroom-1")}
              className="rounded border border-white/10 bg-black/60 px-1.5 py-1 text-[9px] font-bold text-cyan-100"
              aria-label="Global Control System vehicle profile"
            >
              {GLOBAL_CONTROL_DEVICE_PROFILES.map((profile) => (
                <option key={profile.id} value={profile.id}>{profile.displayName} · {profile.vehicleLabel}</option>
              ))}
            </select>
          </label>
        </div>
...
export default FusariumPsathyrellaConsole;
