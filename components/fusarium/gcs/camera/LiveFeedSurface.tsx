...
export interface LiveFeedSurfaceProps {
  ...
  feedId: string;
  /** Operator's AI toggle. The hook is additionally dark unless NEXT_PUBLIC_FUSARIUM_GCS_CAM_AI=1. */
  showAi: boolean;
...
export default function LiveFeedSurface({
   ...
   * queues behind them — `/api/fusarium/gcs/camera` took 15–21 s and link-status/telemetry exceeded
    ...
    <div className={cn("relative h-full w-full overflow-hidden bg-black", className)}>
      ...
      <div className="pointer-events-none absolute bottom-1 left-1 z-20 flex flex-wrap items-center gap-1">
        {!showAi ? null : !ai.enabled ? (
          <span className={cn(chipBase, "border-white/10 bg-black/60 text-slate-500")} title="Set NEXT_PUBLIC_FUSARIUM_GCS_CAM_AI=1 to enable the detector overlay.">
            AI off
          </span>
        ) : !ai.connected ? (
