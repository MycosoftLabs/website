...
import type { BuoyTelemetry, CameraFeed, MagnetometerReading } from "@/lib/fusarium/gcs/contract";
import useSWR from "swr";
import { Compass, Gauge, Wind, Thermometer, Waves, Clock } from "lucide-react";
import VisorFrame from "@/components/fusarium/gcs/camera/VisorFrame";
...
function useRangingReturns(active: boolean): { label: string; m: number; tone?: "cyan" | "amber" }[] {
  ...
  const { data: tof } = useSWR(active ? "/api/fusarium/gcs/fusion-sensors/points" : null, f, o);
  const { data: mmw } = useSWR(active ? "/api/fusarium/gcs/fusion-sensors/mmwave" : null, f, o);
