...
import { GCS_WX_CATALOG } from "./gcs-wx-catalog";
import { isDynamicFieldManifestStale } from "@/lib/crep/fields/field-playback";
...
const FIELD_LAYERS: FieldCfg[] = GCS_WX_CATALOG.map((entry) => ({
  key: entry.key,
  dataset: entry.dataset,
  variable: entry.variable,
  opacity: entry.opacity,
  staticDataset: entry.staticDataset,
}));

function FieldRaster({ dataset, variable, enabled, opacity = 0.7, staticDataset = false }: { dataset: string; variable: string; enabled: boolean; opacity?: number; staticDataset?: boolean }) {
  const { map } = useMap();
  ...
  const stale = isDynamicFieldManifestStale(manifest?.updated, staticDataset);
  const [idx, setIdx] = useState(0);

  // cycle through baked timesteps (image frames only) when the cube is fresh
  useEffect(() => {
    if (!enabled || stale || frames.length < 2) return;
    const t = setInterval(() => setIdx((i) => (i + 1) % frames.length), 900);
    return () => clearInterval(t);
  }, [enabled, stale, frames.length]);
  ...
  useEffect(() => {
    ...
    if (!enabled || stale || !frames.length || !bounds) {
      remove();
      return;
    }
...
export function FieldLayers({ layers }: { layers: LayerState }) {
  return (
    <>
      {FIELD_LAYERS.map((f) => (
        <FieldRaster key={f.key} dataset={f.dataset} variable={f.variable} enabled={!!layers[f.key]} opacity={f.opacity} staticDataset={f.staticDataset} />
      ))}
    </>
  );
}
