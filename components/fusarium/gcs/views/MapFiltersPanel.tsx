...
import { useArraylakeFields } from "@/components/crep/fields/use-arraylake-fields";
import { GCS_WX_CATALOG, GCS_WX_LAYER_IDS } from "./gcs-wx-catalog";
...
const CATEGORIES: { label: string; icon: typeof Waves; layers: { k: LayerKey; label: string; live?: boolean; catalogLayerId?: string }[] }[] = [
  ...
  { label: "Climate & weather", icon: CloudRain, layers: GCS_WX_CATALOG.map((entry) => ({
    k: entry.key,
    label: entry.label,
    catalogLayerId: entry.layerId,
  })) },
...
export function MapFiltersPanel({ layers, onToggle, onReset }: { layers: LayerState; onToggle: (k: LayerKey) => void; onReset?: () => void }) {
  ...
  const { statuses } = useArraylakeFields(open, GCS_WX_LAYER_IDS);
  const statusByLayerId = Object.fromEntries(statuses.map((status) => [status.layerId, status]));

  function weatherFreshness(catalogLayerId?: string): "available" | "stale" | "unavailable" | null {
    if (!catalogLayerId) return null;
    const state = statusByLayerId[catalogLayerId]?.state;
        ...
        {CATEGORIES.map((cat) => {
          ...
          return (
            <div key={cat.label} className="mb-2">
              ...
              {cat.layers.map((l) => {
                const freshness = weatherFreshness(l.catalogLayerId);
                const isCatalogedWx = Boolean(l.catalogLayerId);
                const isLive = isCatalogedWx ? freshness === "available" : l.live === true;
                const freshnessLabel =
                  freshness === "stale" ? "stale" : freshness === "unavailable" ? "unavailable" : null;
                return (
                <button
                  key={l.k}
                  type="button"
                  disabled={!isCatalogedWx && !l.live}
                  ...
                  className={cn(
                    ...
                    !isCatalogedWx && !l.live ? "cursor-not-allowed text-slate-600" : layers[l.k] ? "border-cyan-500/40 bg-cyan-500/15 text-cyan-100" : "text-slate-300 hover:text-white"
                  )}
                >
                  <span>{l.label}</span>
                  {freshnessLabel ? (
                    <span className="text-[8px] uppercase tracking-wide text-amber-400/90">{freshnessLabel}</span>
                  ) : !isLive && !isCatalogedWx ? (
                    <span className="text-[8px] uppercase">soon</span>
                  ) : (
                    <span className={cn("h-2 w-2 rounded-full", layers[l.k] && isLive ? "bg-cyan-400" : "bg-slate-600")} />
                  )}
                </button>
                );
              })}
