...
import { classifyDevice, CONTACT_COLOR, type BuoyTelemetry, type DeviceCategory, type GpsLock, type MapAsset, type MapAssetHover, type SelectedDevice, type SensorContact, type Waypoint } from "@/lib/fusarium/gcs/contract";
...
import { isMapViewActive } from "@/lib/fusarium/gcs/viewState";
  ...
  useEffect(() => {
    ...
    const onMove = (e: any) => {
      ...
      onHover({
        ...
        label: isBuoy ? astr(p.label) || "Global vehicle" : cfg ? cfg.label(p) : id,
    ...
    features: [
      {
        ...
        properties: {
          ...
          color,
          opacity: held ? 0.45 : 1,
          label: held ? `Global vehicle · held ${fmtAge(ageS)}` : fix.lock === "site" ? "Global vehicle · site" : "Global vehicle",
          provenance,
        },
      },
    ],
    ...
    const ensure = () => {
      try {
        if (!map.getSource("psa-buoy")) {
          ...
          map.addLayer({ id: "psa-buoy-label", type: "symbol", source: "psa-buoy", layout: { "text-field": ["coalesce", ["get", "label"], "Global vehicle"], "text-size": 10, "text-font": ["Open Sans Bold"], "text-offset": [0, 1.4], "text-anchor": "top", "text-allow-overlap": true } as any, paint: { "text-color": ["get", "color"], "text-halo-color": "#04070e", "text-halo-width": 1.2 } } as any);
        }
