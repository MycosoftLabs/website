 ...
 *
 * Polls `/api/psathyrella/camera/:feedId/models` and hands back EVERY model's opinion of the same
...
import type { ModelResult } from "@/lib/psathyrella/modelFusion";
...
export function useModelComparison(
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      try {
        const res = await fetch(`/api/psathyrella/camera/${feedId}/models`, { cache: "no-store", signal: ac.signal });
        if (!res.ok) {
          markDisconnected();
        } else {
