...
test("MINDEX upstream unavailable is NOT_SUPPLIED empty, not unbound", () => {
  ...
  assert.match(result.reason, /NOT_SUPPLIED/)
  assert.match(result.reason, /not unbound/)
})
...
test("SporeBase, lab, and Earth-2 empty BFF bodies are no-data, not unbound", () => {
  ...
  const runsOnly = shared.classifyEarth2Spore({ runs: [], count: 0, source: "mas" }, checkedAt)
  const wind = shared.classifyEarth2Wind({ u: [], v: [], source: "none", available: false }, checkedAt)
  assert.equal(sporebase.state, "empty")
  assert.equal(lab.state, "empty")
  assert.equal(modeled.layerId, "modeled-spore-dispersal")
  assert.equal(modeled.state, "empty")
  assert.equal(runsOnly.state, "empty")
  assert.equal(wind.state, "empty")
})
...
test("only delayed, provenance-bearing 15-minute SporeBase tape identifications qualify", () => {
  ...
  assert.equal(pending.state, "empty")
...
test("fungal occurrence stays separate from modeled dispersal and requires source-preserving coordinates", () => {
  ...
  const emptyIngest = shared.classifyFungalOccurrence({ observations: [], meta: { dataSource: "mindex_empty_requires_ingest" } }, checkedAt)
  assert.equal(emptyIngest.state, "empty")
})
...
test("modeled spore zones require a named run and timestamped meteorology driver", () => {
  ...
  assert.equal(withheld.state, "empty")
...
test("AirNow missing configuration is unbound and smoke stays NOT_SUPPLIED empty", () => {
  const airnow = shared.classifyAirNow({ error: "AIRNOW_API_KEY not configured" }, checkedAt)
  assert.equal(airnow.state, "unbound")
  assert.match(airnow.reason, /AIRNOW_API_KEY/)
  const smoke = shared.loadingSharedEarthStatuses(checkedAt).find((item) => item.layerId === "smoke")
  assert.equal(smoke.state, "empty")
  assert.match(smoke.reason, /NOT_SUPPLIED/)
