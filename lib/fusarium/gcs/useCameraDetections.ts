 ...
 *
 * Polls `/api/fusarium/gcs/camera/:feedId/detections` (owner-gated proxy to the Jetson detector) and
...
import type { CameraDetection, DetectionFrame } from "@/lib/fusarium/gcs/contract";
...
export function useCameraDetections(
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      try {
        const res = await fetch(`/api/fusarium/gcs/camera/${feedId}/detections`, { cache: "no-store", signal: ac.signal });
        if (!res.ok) {
