 ...
 *   1. GET /api/fusarium/gcs/telemetry  → the fused BuoyTelemetry envelope (MAS 188)
...
export function useBuoyTelemetry(
  ...
  // sees telemetry only via the stable telemetryRef in MapZone. Disable with NEXT_PUBLIC_FUSARIUM_GCS_SSE=0.
  const streamEnabled = !simulated && process.env.NEXT_PUBLIC_FUSARIUM_GCS_SSE === "1";
