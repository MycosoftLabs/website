...
export const BOUNDS = [[-0.024, -0.022], [0.024, 0.022]];
export const ASSETS = [
...
export function toLngLat(x,y) {return [x/EARTH_RADIUS_M*180/Math.PI,y/EARTH_RADIUS_M*180/Math.PI];}
...
export function snapshot(requestedIndex=0) {
  ...
  ASSETS.forEach((asset,k)=>{
    ...
    const stats={...asset,position,observed_at:latest?.observed_at??null,age_seconds:age,
      current_position_status:age===0?'CURRENT_SYNTHETIC_OBSERVATION':latest?'LAST_OBSERVED_POSITION':'NO_OBSERVATION',
      latest_error_at_observation_m:latest?.error_m??null,nominal_radius_m:RADIUS_M,
    ...
    assets.push(stats);
    features.push(feature('track',asset.id+'-authored-track',{type:'LineString',coordinates:asset.path.map(([x,y])=>toLngLat(x,y))},{label:asset.label+' — preset path',color:asset.color,asset_id:asset.id}));
    if(position){
      features.push(feature('uncertainty',asset.id+'-uncertainty',{type:'Polygon',coordinates:[circle(position,RADIUS_M)]},{label:'Nominal 95% observation-time circle under stated model',color:asset.color,asset_id:asset.id}));
      features.push(feature('asset',asset.id,{type:'Point',coordinates:position},{...stats,path:undefined,color:asset.color,asset_id:asset.id}));
    }
  });
  return {schema:REPLAY_SCHEMA,data_origin:'SYNTHETIC_EXERCISE',execution_mode:'fixed_recorded_simulation',
    ...
    model:{sigma_m:SIGMA_M,nominal_coverage:NOMINAL_COVERAGE,radius_m:RADIUS_M,
      ...
      boundary:'Fixed invented markers and paths near 0°N, 0°E. No connection to supplied OPORD positions, real assets, routing, threat inference, or live feeds. Uncertainty applies at observation time; no extrapolation of stale positions.'},
