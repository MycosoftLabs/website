...
function mindexSource(id, path, result) {
  ...
  if (result.ok && rows === 0) {
    return { id, status: 'NO_DATA', path, http_status: result.status, rows: 0, note: `No data from MINDEX ${path} (source configured; empty result).`, score: null }
  }
...
export async function osmPublicPathways() {
  ...
  const raw = result.ok ? overpassToGeojson(result.data) : { type: 'FeatureCollection', features: [] }
  const features = (raw.features || []).slice(0, 48)
  ...
  if (!result.ok) {
    return {
      ...
      note: `OSM Overpass is configured but the request failed (${result.status}). Source exists; no invented roads.`,
    }
  }
  return {
    id: 'pathways',
    status: rows > 0 ? 'BOUND' : 'NO_DATA',
    source_url: 'https://overpass-api.de/api/interpreter',
    rows,
    geojson,
    note: rows > 0
      ? `${rows} public OSM highway ways in the Fort Stewart bbox (capped at 48). Not official injects.`
      : 'No data from OSM Overpass for the Fort Stewart bbox (source configured; empty result).',
  }
}
...
export async function masDeviceChips() {
  ...
  return {
    ...
    status: chips.length ? 'BOUND' : result.ok ? 'NO_DATA' : 'UNQUALIFIED',
    ...
    note: chips.length
      ...
      : result.ok
        ? 'No data from MAS /api/devices (source configured; empty inventory). Exercise tracks stay synthetic.'
        : `MAS /api/devices request failed (${result.status}). Source exists; inventory not invented.`,
  }
}
...
export async function collectDemoSituationLayers(situation) {
  ...
  const [osmPathways, nominatimBase, masDevices] = await Promise.all([
    byId.pathways?.status === 'BOUND' || byId.pathways?.status === 'SUPPLIED'
      ? Promise.resolve(byId.pathways)
      : osmPublicPathways(),
    byId.equipment_weapons_assets?.status === 'BOUND' || byId.equipment_weapons_assets?.status === 'SUPPLIED'
      ? Promise.resolve(null)
      : osmPublicBase(),
    masDeviceChips(),
  ])
  const traffic = byId.traffic?.status === 'BOUND' || byId.traffic?.status === 'SUPPLIED'
    ? byId.traffic
    : { ...google, id: 'traffic' }
  const pathways = osmPathways
  const navigation = byId.navigation || {
    id: 'navigation',
    status: google.configured ? 'SUPPLIED' : 'NOT_SUPPLIED',
    reason: google.configured ? null : 'google_maps_key_missing',
    note: google.configured
      ? 'Google Maps JS key is configured. Directions/Matrix stay on the website BFF. Not a targeting ETA.'
      : 'google_maps_key_missing. Navigation ETAs are not invented. OSM pathways below are a separate public source.',
  }
  const base = equipment?.status === 'SUPPLIED' || equipment?.status === 'BOUND'
    ...
    : nominatimBase
  const devices = masDevices.chips.length
    ? masDevices
    : {
        id: 'vehicle_constraints',
        status: 'BOUND',
        source_url: 'lib/itdx/replay-core.mjs',
        note: 'No live MAS inventory rows. Exercise uses the four authored synthetic markers — not a weapons catalog.',
        chips: ASSETS_CHIPS(),
      }
  return {
...
function ASSETS_CHIPS() {
  return [
    { id: 'demo-unit-01', label: 'DEMO UNIT 01', kind: 'fictional unit marker', synthetic: true, live: false },
    { id: 'demo-vehicle-02', label: 'DEMO VEHICLE 02', kind: 'fictional transport marker', synthetic: true, live: false },
    { id: 'demo-sensor-03', label: 'DEMO SENSOR 03', kind: 'fictional mobile sensor', synthetic: true, live: false },
    { id: 'demo-air-04', label: 'DEMO AIR 04', kind: 'fictional air marker', synthetic: true, live: false },
  ]
}
