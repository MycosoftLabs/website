...
export function attachReplay(map,{onSelect=()=>{},onStatus=()=>{}}={}) {
  let active=true,index=0,layers={...ALL_KINDS},fusionById={},attached=false,logged=false;
  const buildData=()=>{
    ...
    return {frame,data:filterGeojson(geojson,layers)};
  };
  const install=(reason='attach')=>{
    if(!active||!styleReady(map))return;
    try{
      const {frame,data}=buildData();
      const source=map.getSource(SOURCE_ID);
      if(source)source.setData(data);
      else map.addSource(SOURCE_ID,{type:'geojson',data});
      let added=false;
      for(const layer of LAYERS){
        if(!map.getLayer(layer.id)){map.addLayer(layer);added=true;}
      }
      if(!attached||added||reason==='attach'||reason==='style'){
        attached=true;
        onStatus('LAYER_ATTACHED');
      }
      if(!logged&&typeof console!=='undefined'&&console.info){
        logged=true;
  ...
  const onLoad=()=>install('attach');
  const onStyle=()=>install('style');
  ...
  install('attach');
  return {
    update(next){if(index===next&&attached)return;index=next;install('tick');},
    setFusion(next){fusionById=next&&typeof next==='object'?next:{};install('fusion');},
    setLayers(next){layers={...ALL_KINDS,...next};install('layers');},
    ...
    dispose(){
      active=false;attached=false;map.off('load',onLoad);map.off('style.load',onStyle);map.off('click',SOURCE_ID+'-points',select);
