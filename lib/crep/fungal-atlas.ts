...
function loadLandMask(): LandPolygon[] {
  ...
  try {
    ...
    /* If the Natural Earth mask is unavailable, landPrior still prevents polar/ocean-only extremes. */
  }
...
function extraIslandLandPrior(lat: number, lng: number): boolean {
  const islands: Array<[number, number, number, number]> = [
    [-118.45, 33.39, 0.24, 0.1],  // Catalina Island
    [-118.50, 32.91, 0.22, 0.08],  // San Clemente Island
    [-119.72, 34.0, 0.34, 0.12],   // Channel Islands
    [-117.27, 32.42, 0.08, 0.05],  // Coronado Islands
    [-115.16, 32.28, 0.16, 0.08],  // Isla Angel de la Guarda / Gulf islands signal
  ]
...
function southernCaliforniaWaterMask(lat: number, lng: number): boolean {
  const waterAreas: Array<[number, number, number, number]> = [
    [32.775, -117.225, 0.038, 0.024], // Mission Bay
    [32.706, -117.190, 0.024, 0.068], // San Diego Bay north/south channel
    [32.635, -117.130, 0.036, 0.052], // South Bay
    [32.565, -117.105, 0.024, 0.026], // Tijuana Estuary water
...
function isLikelyLandInPolygons(lat: number, lng: number, polygons: LandPolygon[]): boolean {
  if (lat < -58 || lat > 83.7) return false
  const socal = southernCaliforniaLandMask(lat, lng)
  if (socal !== null) return socal
  if (extraIslandLandPrior(lat, lng)) return true
...
function landPolygonsForBounds(bounds: FungalAtlasBounds): LandPolygon[] | null {
  const polygons = loadLandMask()
...
function valueAt(cache: AtlasCache, lat: number, lng: number, layer: FungalAtlasLayer, _zoom: number, landPolygons?: LandPolygon[] | null, skipLandMask = false): number {
  if (!skipLandMask && (landPolygons ? !isLikelyLandInPolygons(lat, lng, landPolygons) : !isLikelyLand(lat, lng))) return 0
