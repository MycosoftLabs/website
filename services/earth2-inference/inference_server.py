...
class Settings(BaseModel):
    ...
    physics_api_url: str = Field(default="http://localhost:8400")
...
settings = Settings(
    ...
    physics_api_url=os.environ.get("PHYSICSNEMO_API_URL", "http://localhost:8400"),
)
...
class DataSourceManager:
    ...
    async def _fetch_gfs(
        ...
        try:
            ...
            raise HTTPException(status_code=503, detail="GFS datasource unavailable in this environment")
    ...
    async def _fetch_era5_arco(
        ...
        try:
            ...
            raise HTTPException(status_code=503, detail="ERA5 ARCO datasource unavailable in this environment")
    ...
    async def _fetch_hrrr(
        ...
        try:
            ...
            raise HTTPException(status_code=503, detail="HRRR datasource unavailable in this environment")
...
class InferenceEngine:
    ...
    async def _run_model_inference(
        ...
        try:
            ...
            raise HTTPException(
                status_code=503,
                detail="Earth2Studio runtime is unavailable; no synthetic fallback is used",
            )
...
async def run_downscale(request: DownscaleRequest):
    ...
    # Run downscaling and return model status. Raw fields are generated by CorrDiff.
    return {
        "status": "completed",
        "engine": "corrdiff",
        "target_resolution": request.target_resolution,
        "bounds": request.bounds.model_dump(),
        "message": "CorrDiff downscaling request completed",
    }


@app.post("/physics-correction")
async def run_physics_correction(request: ForecastRequest):
    """
    Apply PhysicsNeMo correction against forecast summary statistics.
    """
    forecast = await inference_engine.run_forecast(request)
    try:
        correction_payload = {
            "equation": "diffusion",
            "x": [float(i) for i in range(len(forecast.steps))],
            "y": [float(i) for i in range(len(forecast.steps))],
            "boundary_value": 0.0,
            "source_term": 1.0,
        }
        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{settings.physics_api_url.rstrip('/')}/physics/pinn",
                json=correction_payload,
                timeout=aiohttp.ClientTimeout(total=20),
            ) as response:
                if response.status >= 400:
                    detail = await response.text()
                    raise HTTPException(status_code=502, detail=f"Physics correction failed: {detail}")
                correction = await response.json()
    except HTTPException:
        raise
    except Exception as exc:
        raise HTTPException(status_code=502, detail=f"Physics correction unavailable: {exc}") from exc

    return {
        "status": "completed",
        "forecast": forecast.model_dump(),
        "physics_correction": correction,
        "engine": "earth2+physicsnemo",
    }
