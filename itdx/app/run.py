...
from itdx import source_workspace
...
class Application:
    def __init__(self,data_dir):
        ...
        self.source_packs=source_workspace.initialize(ROOT,self.store)
 ...
 class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            ...
            if path=='/api/workspace':return self.send(source_workspace.state(ROOT,app.store,app.source_packs))
            if path=='/api/workspace/search':
                docs=app.store.list('documents',2000)
                if q.get('scope',['all'])[0]=='exercise':docs=[d for d in docs if d.get('pack_id')]
                return self.send({'hits':search_documents(docs,q.get('q',[''])[0],limit=40)})
            if path=='/api/workspace/page':return self.send(source_workspace.page_record(app.store,q['id'][0],int(q.get('page',['1'])[0])))
            if path=='/api/workspace/preview':
                d=app.store.get('documents',safe_id(q['id'][0]));number=int(q.get('page',['1'])[0]);source_workspace.page_record(app.store,d['id'],number)
                if not d.get('pack_id') or not d['pages'][number-1].get('preview_file'):raise KeyError('Preview not available')
                base=(ROOT/'exercise_packs'/safe_id(d['pack_id'])).resolve();file=(base/d['pages'][number-1]['preview_file']).resolve()
                if not file.is_relative_to(base):raise ValueError('Invalid preview path')
                return self.send(file.read_bytes(),ctype='image/jpeg')
            if path=='/api/workspace/export':
                fmt=q.get('format',['json'])[0]
                if fmt=='zip':return self.send(source_workspace.bundle(ROOT,app.store,app.source_packs),ctype='application/zip',filename='itdx-source-workspace.zip')
                if fmt!='json':raise ValueError('Unknown source export format')
                return self.send(source_workspace.portable(ROOT,app.store,app.source_packs),filename='itdx-source-workspace.json')
            ...
            if path=='/api/document/download':
                d=app.store.get('documents',safe_id(q['id'][0]));file=source_workspace.source_path(ROOT,app.store,d)
            ...
            return self.send(file.read_bytes(),ctype={'.js':'text/javascript; charset=utf-8','.mjs':'text/javascript; charset=utf-8','.css':'text/css; charset=utf-8','.html':'text/html; charset=utf-8'}.get(file.suffix,mimetypes.guess_type(file.name)[0] or 'application/octet-stream'))
    ...
    def do_POST(self):
        try:
            ...
            if path=='/api/workspace/notes':return self.send(source_workspace.save_note(app.store,body))
            if path=='/api/workspace/check':return self.send(source_workspace.check_documents(ROOT,app.store))
            if path=='/api/ranking':
