...
class Store:
    ...
    def put(self,table,identifier,payload,**extra):
        if table not in ['datasets','documents','runs','suites','rankings','civil_cases','settings']:raise ValueError('Invalid collection')
    ...
    def list(self,table,limit=200):
        if table not in ['datasets','documents','runs','suites','rankings','civil_cases']:raise ValueError('Invalid collection')
