...
class LauncherTests(unittest.TestCase):
 ...
 def test_browser_dispatch_waits_for_verified_server(self):
  ...
  with tempfile.TemporaryDirectory() as d:
   ...
   self.assertEqual(code,0);self.assertEqual(len(calls),1);self.assertEqual(calls[0]['documents'],30);self.assertFalse((Path(d)/'ready.json').exists())
 ...
 def test_real_launcher_starts_and_stops_healthy_app_on_fallback_port(self):
  with socket.socket() as occupied,tempfile.TemporaryDirectory() as d:
   ...
   self.assertEqual(p.returncode,0,p.stdout+p.stderr);self.assertIn('APP IS READY:',p.stdout);self.assertIn('30 documents | 41 scenarios',p.stdout);self.assertIn('is occupied. Using',p.stdout);self.assertIn('Startup check passed',p.stdout);self.assertFalse((Path(d)/'ready.json').exists())
