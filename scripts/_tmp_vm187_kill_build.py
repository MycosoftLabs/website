...
def main() -> None:
    ...
    cmds = [
        "pkill -f 'docker build --network host --no-cache -t mycosoft-always-on-mycosoft-website:latest' || true",
        "pkill -f '_rebuild_sandbox.py' || true",
        "ps -ef | grep -E 'docker build|_rebuild_sandbox' | grep -v grep || true",
    ]
    for cmd in cmds:
