#!/usr/bin/env python3
"""Apply compounds migration with corrected schema (integer taxon IDs)."""
import psycopg2
import sys
...
-- Taxon-compound relationship (using INTEGER for taxon_id to match core.taxon)
CREATE TABLE IF NOT EXISTS bio.taxon_compound (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    taxon_id integer NOT NULL REFERENCES core.taxon (id) ON DELETE CASCADE,
...
INSERT INTO bio.biological_activity (name, category, description) VALUES
    ...
    ('Immunomodulating', 'immunology', 'Modulates immune system'),
...
def main():
    try:
        print(f"[*] Connecting to MINDEX DB...")
        ...
        print(f"[+] bio.compound: {compound_count} rows")
        
        cur.execute("SELECT COUNT(*) FROM bio.genetic_sequence")
        genetics_count = cur.fetchone()[0]
        print(f"[+] bio.genetic_sequence: {genetics_count} rows")
        
        cur.execute("SELECT COUNT(*) FROM bio.biological_activity")
        activity_count = cur.fetchone()[0]
        print(f"[+] bio.biological_activity: {activity_count} rows")
        
        cur.close()
        conn.close()
        
        print("\n[+] Compounds schema created successfully")
        return 0
