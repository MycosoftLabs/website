...
def main() -> None:
    ...
    command = (
        "asset=$(find /opt/mycosoft/media/website/assets -type f | head -n 1); "
        "if [ -z \"$asset\" ]; then echo \"no-asset\"; exit 0; fi; "
        "rel=${asset#/opt/mycosoft/media/website/assets}; "
        "echo \"$rel\"; "
        "curl -s -o /dev/null -w '%{http_code}' http://localhost:3000/assets${rel}"
    )
